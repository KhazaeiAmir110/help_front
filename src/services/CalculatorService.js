 ...
 *
 * This service computes the payment weight, making_charges value, markup amount, and final payment
 * based on the specified spot price, net weight, additional attachments, making_charges percentage,
 * and markup percentage.
 *
 * @param {number} [spotPrice=4500000] - The price per unit of the spot (default is 4500000).
 ...
 * @param {number} [making_charges=18] - The percentage of making_charges (default is 18).
 ...
 * - paymentHallmark: The computed making_charges value.
 * - paymentMarkup: The computed markup amount.
 * - paymentFinal: The final calculated payment amount.
 */
function CalculatorService(spotPrice = 4500000, netWeight = 1, attachments = 0,
                           making_charges = 18, markup = 7) {
    ...
    function Services(spotPrice, netWeight, attachments, making_charges, markup) {
        const payment_weight = Math.ceil(spotPrice * netWeight);
        const payment_hallmark = Math.ceil((making_charges / 100) * payment_weight);
    ...
    useEffect(() => {
        Services(spotPrice, netWeight, attachments, making_charges, markup);
    }, [spotPrice, netWeight, attachments, making_charges, markup]);
